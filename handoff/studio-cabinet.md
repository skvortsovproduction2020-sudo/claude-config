...
**0. Проверить загрузку файла в «Переговоры»** (приоритет 1)
   - Попробовать загрузить реальную запись созвона
   - Если ошибка 403 → создать политику в Supabase SQL Editor (SQL выше в хэндофе)
   - Если работает → проверить полный флоу: загрузка → расшифровка → протокол → анализ → КП → PDF
