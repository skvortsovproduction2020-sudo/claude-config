...
## 📍 ТЕКУЩЕЕ СОСТОЯНИЕ
*Обновлено: 2026-06-24 (сессия 21)*

- [x] **Диаризация спикеров в переговорах** — после транскрипции Groq Claude Haiku размечает [ПРОДЮСЕР:] / [КЛИЕНТ:] по чанкам; промпты protocol и analyze знают кто есть кто
- [x] **Кнопка Я.Диск для замены записи** — когда транскрипт уже есть, появляются кнопки «Я.Диск» и «Файл» вместо одной «Заменить»
- [x] **Профиль продюсера — кейсы** — колонка `producer_cases` в user_settings; скрипт `extract-producer-cases.mjs` вытащил из переговоров/данных: 8 кейсов, цифры (700K подпис., 500М просмотров — Гусев), 6 сильных сторон; вставлено в промпт КП
- [x] **Subtitle убран из КП** — `display:none` в CSS + инструкция в промпте; больше не нужно удалять вручную
- [x] **Фикс JSON парсинга переговоров** — protocol/analyze/prep теперь вытаскивают `{…}` regex перед parse; логируют raw-ответ при ошибке
...
## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-24 сессия 21)

**Что сделали — анализ Telegram + кнопка повтора зависших задач + amoCRM токен:**

1. **Кнопка «Повторить» для зависших задач** — `app/dashboard/style/page.js`:
   - Раньше кнопка «Повторить» показывалась только при `status='error'`
   - Теперь показывается также при `isProcessing && elapsedMin >= 20` (висит в обработке >20 мин)
   - `elapsedMin` считается из `processing_started_at || created_at`

2. **Фикс анализа Telegram-канала san_autoparts** — `app/api/telegram-analyze/route.js`:
   - Канал `san_autoparts` редиректит `t.me/s/san_autoparts` → `t.me/san_autoparts`. Редиректный HTML содержит `tgme_page_title` (TRUE) но не содержит `tgme_widget_message` (FALSE)
   - Старое условие `FALSE && TRUE = FALSE` не ловило ситуацию «канал найден, но нет превью»
   - Исправлено: разбили на два отдельных `if` — сначала проверка заголовка канала, потом наличия постов
   - Добавлен depth-counting парсер вместо старого regex (Telegram меняет структуру HTML)
   - Добавлены browser-like заголовки (User-Agent, Accept, Sec-Fetch-*)

3. **Apify-интеграция для каналов без веб-превью** — новый файл `app/api/apify-telegram/route.js`:
   - POST `{ channel }` → запускает `clockworks~telegram-scraper` через Apify API → возвращает `{ runId }`
   - POST `{ runId }` → опрашивает статус → при SUCCEEDED → возвращает `{ posts }`
   - `app/dashboard/telegram/page.js` — автоматический fallback: прямой скрейп → если ошибка «не поддерживает веб-превью» → запускает Apify → polling каждые 5 сек → когда получены посты → отправляет в `telegram-analyze` через `{ messages: posts }` вместо `{ channel }`
   - Прогресс: «Запускаю Apify скрейпер...» → «Apify собирает посты...» → «Нашёл X постов, анализирую через Claude...»

4. **amoCRM токен — найден правильный путь**:
   - Не через amoМАРКЕТ как думали, а через прямую ссылку: `https://sanautoparts2.amocrm.ru/settings/integrations/`
   - Там: «+ Создать интеграцию» → «Внешняя» → сохранить → вкладка «Ключи и доступы» → Долгосрочный токен
   - Токен ещё не получен (пользователь не дошёл до конца в этой сессии)

**Что не сработало:**
- amoМАРКЕТ «...» показывает только фильтры категорий, кнопки «Создать интеграцию» там нет

**Новые/изменённые файлы:**
```
app/dashboard/style/page.js           — кнопка «Повторить» при isProcessing && >20 мин
app/api/telegram-analyze/route.js     — depth-counting парсер, browser headers, два отдельных if
app/api/apify-telegram/route.js       — НОВЫЙ: Apify clockworks~telegram-scraper
app/dashboard/telegram/page.js        — auto-fallback на Apify при ошибке веб-превью
```

---

## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-23 сессия 20)

**Что сделали — фикс retry при скачивании с Яндекс.Диска:**

1. **Причина бага «нет данных 90 сек»** — Яндекс.Диск периодически прерывает соединение при скачивании больших файлов. Бот падал при первом же обрыве. Части 4, 6, 9 слёта нарывались на такие обрывы.

2. **Фикс в `bot.js`** — добавлена retry-логика в `processVoiceSource`:
   - До 3 попыток скачать каждый файл
   - Между попытками: 15с / 30с пауза
   - Каждый раз берётся свежая ссылка с Яндекс.Диска (старые истекают)
   - При провале всех 3 попыток — ошибка как раньше

3. **Обновлён `CLAUDE.md`** — пайплайн Яндекс.Диска теперь отражает 300с watchdog + 3 retry (было написано "90 сек", что было устаревшим).

4. **Задеплоено** — `git push` → Railway поднял новую версию бота (~2 мин).

**Что нужно сделать после деплоя:**
- Нажать «Повторить» на частях 4, 6, 9 слёта 2026_06_11 в Анализе данных

**Новые/изменённые файлы:**
```
bot.js   — retry-логика в processVoiceSource (MAX_DL_ATTEMPTS = 3)
CLAUDE.md — обновлён пайплайн Яндекс.Диска
```

---

## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-22 сессия 19)

**Что сделали — диаризация, профиль продюсера, фиксы КП:**

1. **Диаризация спикеров** — `diarizeTranscript()` в bot.js: после Groq отправляет транскрипт в Claude Haiku чанками по 25K символов, получает разметку [ПРОДЮСЕР:] / [КЛИЕНТ:], сохраняет. Промпты `protocol/route.js` и `analyze/route.js` обновлены — объясняют Claude кто есть кто.

2. **Кнопка Я.Диск для замены** — в `app/crm/negotiations/[id]/page.js`: когда транскрипт уже есть, вместо одной кнопки «Заменить» появляются «Я.Диск» (открывает поле для ссылки) и «Файл».

3. **Профиль продюсера** — добавили колонку `producer_cases JSONB` в `user_settings`. Скрипт извлёк из переговоров и данных: 8 кейсов (Гусев — 700K подпис./500М просмотров, Вячеслав Севрюков, ПроАвто, Ладушки, Cream Shop, Анна Громова, Отец Серафим, Славиогло), 6 сильных сторон, специализацию, подход. Всё инжектируется в промпт генерации КП.

4. **Subtitle убран из КП** — `display:none` в CSS шаблоне + инструкция в промпте.

5. **Фикс JSON парсинга** — в protocol/analyze/prep route.js: `raw.match(/\{[\s\S]*\}/)` + логирование при ошибке.

**⚠️ Важно — Мария Зикрат:**
- Транскрипт в БД **обрезан до ~19K** из-за ошибки первого скрипта диаризации (лимит вывода Haiku)
- Оригинал (211K) виден на скриншоте браузера (кеш), но в БД уже 19K
- Нужно загрузить запись заново через кнопку «Я.Диск» — тогда бот расшифрует полностью + диаризует по чанкам

**Новые/изменённые файлы:**
```
bot.js                                        — diarizeTranscript() по чанкам
app/crm/negotiations/[id]/page.js             — кнопки Я.Диск + Файл при замене
app/api/negotiations/[id]/kp/route.js         — producer_cases в промпте, subtitle убран
app/api/negotiations/[id]/protocol/route.js   — jsonMatch парсинг + метки спикеров в промпте
app/api/negotiations/[id]/analyze/route.js    — jsonMatch парсинг + метки спикеров в промпте
app/api/negotiations/[id]/prep/route.js       — jsonMatch парсинг
```

## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-21 сессия 18)

**Что сделали — поддержка папок и аудио в переговорах + Railway Hobby:**

1. **Поддержка папок Яндекс.Диска в переговорах** — `processNegotiation` переписан:
   - Вызывает `resolveYandexDiskItems` с расширенным фильтром (видео + аудио: m4a, mp3, wav, ogg, aac, opus, flac)
   - Папка → обрабатывает все файлы по очереди, транскрипты склеиваются в один (каждая часть помечена `[filename]`)
   - Одиночный файл → работает как раньше
   - Watchdog увеличен с 90 до 300 сек

2. **Фикс расширения временного файла** — `path.extname(item.name)` добавляется к имени файла (`neg_xxx_0_src.m4a`). Без расширения ffmpeg не мог определить формат M4A и падал с "Invalid data found".

3. **Railway Trial закончился** — деплой молча не работал ~30 минут. Обновились на **Hobby план ($5/мес)**. Задеплоили через `railway up --detach`.

**⚠️ Статус:** деплой был в состоянии Building в конце сессии. Нужно проверить что расшифровка заработала.

**Новые/изменённые файлы:**
```
bot.js — resolveYandexDiskItems(extFilter), processNegotiation (папки + аудио + расширение файла)
```

## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-21 сессия 17)

**Что сделали — Трекер: TG-привязка через код + спиннер кнопки:**

1. **Кнопка "Из TG" — спиннер** — при нажатии сразу показывает панель + иконка Loader2 + текст "Загружаю…". Раньше ничего не происходило визуально до конца загрузки.

2. **link_code — технический код привязки группы** — вместо `/link Имя Фамилия`:
   - Миграция `migrate-tracker-v3.mjs`: поле `link_code TEXT UNIQUE` в `tracker_clients`, заполнено для всех существующих через `gen_random_uuid()` (первые 6 символов hex)
   - Бот: `/link КОД` — ищет по `link_code`, без зависимости от имён и статусов
   - API создания клиента: генерирует 6-символьный код из `ABCDEFGHJKLMNPQRSTUVWXYZ23456789` (без O, I, 0, 1)
   - UI (вкладка Профиль): кнопка-код `/link XXXXXX` — нажал, скопировал, вставил в группу; при копировании иконка меняется на Check на 2 сек; показывает "✅ группа привязана" если tg_chat_id заполнен

3. **Debug бота** — упрощён regex на `/\/link/i` + добавлен `console.log` + `try/catch` с ответом об ошибках. После отладки нужно вернуть строгий regex.

4. **Привязка сработала** — группа "Трекинг продвижения Шитикова Ладушки" привязана к Анне Шитиковой. Панель "Из TG" показывает "Новых сообщений нет" (вместо "Бот не добавлен").

5. **Задачи зафиксированы в handoff** — HR CRM + Брендинг "Скворцов Продакшн".

**Новые/изменённые файлы:**
```
app/tracker/[id]/page.js              — спиннер кнопки, link_code в профиле, copiedCode state
app/api/tracker/clients/route.js      — генерация link_code при создании
bot.js                                — /link КОД обработчик (debug regex)
migrate-tracker-v3.mjs               — ЗАПУЩЕНА ✅ (link_code поле)
```

**⚠️ Нужно сделать в следующей сессии:**
- Убрать debug из бота: вернуть строгий regex `/^\/link(?:@\w+)?(?:\s+(\S+))?/i` и убрать `console.log`
- Проверить что сообщения из группы сохраняются и появляются в панели

---

## ✅ ПОСЛЕДНЯЯ СЕССИЯ (2026-06-20 сессия 16)

**Что сделали — Reels-тайм-коды с именами файлов + авто-обновление профиля стиля:**

1. **Фикс timeout «Найти Reels»** — `analyze-reels/route.js` переписан: обрабатывает ОДИН источник за вызов (~10 сек вместо 90+). Фронт в цикле вызывает API пока все не готовы. Кнопка показывает прогресс «Ищу… 3/9».

2. **Авто-обновление профиля стиля** — в `startPoll()` добавлен вызов `triggerStyleUpdate()` (debounce 3 сек) когда источник переходит в статус `done`. Больше не нужно жать «Обновить профиль» вручную.

3. **Имена файлов в Reels-моментах** — ключевое улучшение:
   - **bot.js**: после обработки папки Яндекс.Диска сохраняет `file_transcripts: [{filename, text}]` в voice_sources
   - **analyze-reels/route.js**: если у источника есть `file_transcripts` → анализирует КАЖДЫЙ файл отдельно → в каждом моменте `file: "MVI_4665.MP4"` и тайм-код ВНУТРИ этого файла (не от начала пакета)
   - Для старых источников без `file_transcripts` → fallback на общий транскрипт (как раньше)
   - Миграция: `migrate-voice-file-transcripts.mjs` → `voice_sources.file_transcripts JSONB`

4. **skip_ids в analyze-reels** — фронт передаёт массив source_ids у которых была ошибка Claude, API их пропускает → нет бесконечного цикла на проблемных источниках.

**Статус загрузки слёта:**
- Части 1-2: обработаны ДО обновления бота → reels_moments найдены через «Найти Reels» (approx, без имён файлов)
- Части 3-9: обрабатываются с НОВЫМ ботом → получат `file_transcripts` + exact reels_moments с именами файлов (зелёные)
- Часть 8: ошибка «нет данных 90 сек», нажать «Повторить»

**Новые/изменённые файлы:**
```
app/api/voice-base/analyze-reels/route.js  — per-file анализ, skip_ids, one-at-a-time
app/dashboard/style/page.js               — цикл Reels, авто-профиль, прогресс в кнопке
bot.js                                    — сохраняет file_transcripts при done
migrate-voice-file-transcripts.mjs        — ЗАПУЩЕНА ✅
```

**Архитектура voice_sources после этой сессии:**
```
file_transcripts: [{filename: "MVI_4665.MP4", text: "транскрипт..."}]
reels_moments:   [{file: "MVI_4665.MP4", start_fmt: "2:15", end_fmt: "3:00", quote, reason, approx?}]
```
...
### ➡️ Следующий шаг (после сессии 21)

**0. amoCRM токен** — зайти по `https://sanautoparts2.amocrm.ru/settings/integrations/` → «+ Создать интеграцию» → «Внешняя» → сохранить → вкладка «Ключи и доступы» → скопировать Долгосрочный токен → вставить в Studio Cabinet → Анализ данных → CRM.

**1. Протестировать Apify на san_autoparts** — зайти на вкладку Telegram, вставить `san_autoparts`, нажать «Анализировать». Должен автоматически переключиться на Apify и показать прогресс.

**2. Нажать «Повторить» на зависших частях слёта** — если части 4, 6, 9 до сих пор висят в processing >20 мин — кнопка «Повторить» теперь должна появиться.

### ➡️ Следующий шаг (после сессии 19)

**0. Загрузить запись Марии Зикрат заново** — CRM → Переговоры → Мария Зикрат → вкладка «Запись» → кнопка «Я.Диск» → вставить ссылку. Бот расшифрует полностью + диаризует. Транскрипт сейчас обрезан.

**1. Убрать debug из бота** — заменить `/\/link/i` обратно на `/^\/link(?:@\w+)?(?:\s+(\S+))?/i`, убрать console.log и debug-ответы. Привязка Трекера уже работает, debug больше не нужен.

**2. Проверить TG-сообщения** — написать что-нибудь в группу Анны, нажать "Из TG" — сообщение должно появиться в панели.

**🆕 HR CRM — вкладка для сотрудников (запланировано)**
- Отдельный раздел (аналог CRM для клиентов, но для команды)
- Карточка сотрудника: собеседования + AI-анализ (скрытые мотивы, характер, можно ли работать)
- Созвоны с сотрудниками → расшифровка + AI-сводка (как в трекерстве)
- Базовый HR: статусы (кандидат / оффер / в команде / ушёл), должность, роль
- Задачник студии: задачи по компании (найм монтажёра, ответственный, дедлайн) — прямо в HR CRM

**🆕 Брендинг "Скворцов Продакшн" (запланировано)**
- Переименование платформы: Studio Cabinet → Скворцов Продакшн
- Фирменный стиль: счета, договора, КП — лого, подпись, верхний колонтитул
- Стиль «контент / личный бренд»: элегантно, не корпоративно
- Фирменная подпись в документах (красиво, с указанием направления)

---

**0. Дождаться частей 3-9** — когда Railway обработает все части слёта, нажать «Найти Reels» для частей у которых ещё нет моментов. Части 3-9 уже получат exact timecodes с именами файлов автоматически (бот делает это при обработке).

**1. Часть 8** — нажать «Повторить» (ошибка watchdog 90 сек, бот уже обновлён на 300 сек).

**2. Проставить group_name для частей 1-2** — они созданы без группы. Нужно либо редактировать через Supabase напрямую, либо сделать UI для inline-редактирования group_name в карточке источника.

---

**3. Удалить старые страницы** (после проверки что /crm работает — уже проверено, можно удалять):
- `app/dashboard/negotiations/` → удалить
- `app/dashboard/invoices/` → удалить

**1. Публичный роут КП для контакта** — `/kp/contact/[id]` (сейчас есть только `/kp/[id]` для встречи). Кнопка «Поделиться» в вкладке КП карточки контакта ведёт на несуществующий роут.

**2. Запустить migrate-tracker.mjs** — таблицы трекера созданы в коде, но миграция должна быть запущена: `cd C:\Users\89117\studio && node migrate-tracker.mjs`

**3. Создать договор для Анны (трекерство)** — CRM → Договоры → Новый договор → Трекерство → реквизиты Анны, 2 мес, 2 встречи, 50 000 ₽/мес → PDF

**4. amoCRM — подключить первый проект** — Студия → Анализ данных → CRM, ввести поддомен и токен клиента (ПроАвто / Гусев)

**5. «Получено» в кабинете продюсера** — определить источник данных

**6. Раздел «Отчёты»** — страница: данные по воронке → Claude генерирует HTML-презентацию (образец: `C:\Users\89117\proavto-report.html`)

**7. Интеграция SaleBot → CRM** (отложено) — ждёт названий переменных (статус, предоплата, отказ)
