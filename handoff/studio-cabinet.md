...
**1. Создать договор для Анны (трекерство)** — Оплата и договор → Новый договор → Трекерство → заполнить реквизиты Анны, 2 мес, 2 встречи, 50 000 ₽/мес → скачать PDF

**2. Проверить поле «Итог встречи»** — Переговоры → Анна → встреча → вкладка «Запись» → вписать итог → нажать «Сгенерировать КП» → убедиться что итог учтён

**3. amoCRM — подключить первый проект** — Анализ данных → CRM, ввести поддомен и токен клиента (ПроАвто / Гусев), нажать «Синхронизировать»

**4. «Получено» в кабинете продюсера** — определить источник данных (из CRM / банка / ручной ввод) и реализовать

**5. Раздел «Отчёты»** — страница: вводишь данные по воронке → Claude генерирует HTML-презентацию (образец: `C:\Users\89117\proavto-report.html`)
