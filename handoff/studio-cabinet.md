...
### ➡️ Следующий шаг (после сессии 17)

**0. Убрать debug из бота** — заменить `/\/link/i` обратно на `/^\/link(?:@\w+)?(?:\s+(\S+))?/i`, убрать console.log и debug-ответы. Привязка уже работает, debug больше не нужен.

**1. Проверить TG-сообщения** — написать что-нибудь в группу Анны, нажать "Из TG" — сообщение должно появиться в панели и можно добавить как задание.
